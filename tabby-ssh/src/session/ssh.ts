...
export class SSHSession {
    ...
    async init (): Promise<void> {
        ...
        if (!this.profile.options.auth || this.profile.options.auth === 'publicKey') {
            if (this.profile.options.privateKeys && this.profile.options.privateKeys.length) {
    ...
    async start (): Promise<void> {
        ...
        this.ssh = await russh.SSHClient.connect(
            ...
            },
            {
                ...
                keepaliveIntervalSeconds: Math.round((this.profile.options.keepaliveInterval ?? 5000) / 1000),
                keepaliveCountMax: this.profile.options.keepaliveCountMax ?? 10,
